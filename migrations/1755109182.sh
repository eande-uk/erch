...
# Users can still opt-in to Cloudflare DNS using: erch-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-erch-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-erch-dns.network
  ...
  echo "To use Cloudflare DNS, run: erch-setup-dns Cloudflare"
fi