...
# Move user-added backgrounds from erch theme folders to user config
ERCH_DIR="$HOME/.local/share/erch"
USER_BACKGROUNDS_DIR="$HOME/.config/erch/backgrounds"

if [[ -d $ERCH_DIR/themes ]]; then
  cd "$ERCH_DIR"

  # Get list of git-tracked background files (relative to erch dir)
...
THEMES_DIR="$HOME/.config/erch/themes"
CURRENT_THEME_LINK="$HOME/.config/erch/current/theme"
...
elif [[ -f $HOME/.config/erch/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/erch/current/theme.name")
fi

# Remove all symlinks from ~/.config/erch/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  erch-theme-set "$CURRENT_THEME_NAME"
else
