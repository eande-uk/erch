...
if $NEEDS_MIGRATION; then
  ...
  erch-pkg-drop walker-bin walker-bin-debug

  erch-pkg-add "${PACKAGES[@]}"

  source $ERCH_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  erch-refresh-config walker/config.toml
  erch-refresh-config elephant/calc.toml
  erch-refresh-config elephant/desktopapplications.toml
fi
