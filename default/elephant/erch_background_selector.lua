Name = "erchBackgroundSelector"
NamePretty = "erch Background Selector"
Cache = false
...
function GetEntries()
  ...
  local theme_name_file = io.open(home .. "/.config/erch/current/theme.name", "r")
  ...
  local dirs = {
    home .. "/.config/erch/current/theme/backgrounds",
  }
  if theme_name then
    table.insert(dirs, home .. "/.config/erch/backgrounds/" .. theme_name)
  end
      ...
      for background in handle:lines() do
        ...
        if filename and not seen[filename] then
          ...
          table.insert(entries, {
            ...
            Value = background,
            Actions = {
              activate = "erch-theme-bg-set " .. ShellEscape(background),
            },
            Preview = background,
            PreviewType = "file",
          })
