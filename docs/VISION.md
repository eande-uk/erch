...
erch is a beautiful, modern, packaged-by-use-case Linux distribution by E&E. It is a self-contained fork of Omarchy — arch Linux + Hyprland — that ships the complete desired desktop configuration natively, from bare metal to fully polished, without requiring any external repo.

## Guiding Principles

1. **Source of truth.** erch owns all system commands, configs, themes, and install pipeline. The parent repo (`eande-os`) stays target-agnostic, supporting erch, upstream Omarchy, and bare Arch+Hyprland from a single dotfiles mirror.
...
5. **Everything managed via `erch`.** A single CLI dispatches to 290+ commands across 30+ groups. Self-documenting metadata, tab completion, machine-readable JSON output.
